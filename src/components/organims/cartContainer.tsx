...
const CartContainer = (props:Basket) => {
      ...
      {props?.items?.length === 0 || props?.items === undefined
        ...
        <>
          ...
          <div className=' w-full flex justify-between h-[60px] border-t-[#DADADA] border-t-[1px]  bg-gray-50 items-center p-4'>
            ...
            <Typography className="text-base font-medium">{props.totalPrice}</Typography>
          </div>
          <div className=' w-full flex justify-between h-[60px] border-t-[#DADADA] border-t-[1px]  bg-gray-50 items-center p-4'>
            <Typography className="text-2xl font-light">{t('cart.total')}</Typography>
            <Typography className="text-2xl font-normal">{props.totalPrice}</Typography>
          </div>
        </>
      }
    </aside>
