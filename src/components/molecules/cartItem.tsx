...
import { addItemQuantity } from '../../store/action'

const CartItem = (props:BasketItem) => {
    ...
    <div className='flex items-center justify-between flex-shrink-0 w-full'>
      <div className=' w-full flex min-h-16 min-w-[78px] flex-col items-start'>
        <div className='flex items-center justify-between w-full '>
          <Typography className="text-base font-normal">{props.name}</Typography>
          <Typography className="text-base font-normal" currency>{props.price}</Typography>
        </div>
        ...
        <div className="flex items-center justify-between min-w-[78px] w-min h-5 mt-2 ml-2">
          <button
            onClick={() => dispatch(addItemQuantity(-1, props.id))}
            className="flex items-center justify-center w-5 h-5 rounded-full bg-burgerBrown"
          >
            <div className="bg-white w-3 h-[3px] rounded"></div>
          </button>
          <span className="text-base font-bold">{props.quantity}</span>
          <button
            onClick={() => dispatch(addItemQuantity(1, props.id))}
            className="flex items-center justify-center w-5 h-5 rounded-full bg-burgerBrown"
          >
